...
REVIEW_AI_SUMMARY = [
    "В AI-проверенный корпус за июнь попало 78 обращений — все за первую декаду месяца "
    "(2–9 июня), после 9 июня данных в корпусе нет, поэтому сводка охватывает именно этот "
    "период. Общий фон напряжённый: 62% обращений негативные, 34 (44%) — повышенной "
    "остроты (4–5).",
    "Главная тема декады — «Дороги и транспорт» (21 обращение, 27% корпуса). Горожане "
    ...
    "Жетысуский, Алмалинский, Медеуский) — по остальным районам данных за июнь недостаточно "
    "для честной детализации, это помечено на карте знаком «❔».",
]
...
def month_review(posts):
    ...
    return {
        ...
        "n": n, "reach": sum(engagement(p) for p in mp),
