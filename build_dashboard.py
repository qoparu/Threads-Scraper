...
# Раньше триггер-слово и слово-подсказку "человек" искали НЕЗАВИСИМО в любом
# месте текста — на длинных новостных постах это ловило что угодно (например,
# «история о пропавших ЧАСАХ» + случайное упоминание «человека» абзацем ниже).
# Теперь требуем, чтобы они стояли РЯДОМ (в любом порядке) — это фактическая
# формулировка «пропал(а) человек/девушка/...», а не совпадение по всему тексту.
_MISSING_ROOT = (r"(?:пропал\w*|пропавш\w*|без\s+вести|разыскивается|в\s+розыске|"
                 r"объявлен\w*\s+в\s+розыск|потерял[ас]ся|жоғалды|іздеуде)")
_PERSON_WORD = (r"(?:человек\w*|мужчин\w*|женщин\w*|девушк\w*|парен\w*|ребен\w*|ребён\w*|"
                r"брат\w*|сестр\w*|сын\w*|дочь\w*|дочк\w*|дедушк\w*|бабушк\w*|родствен\w*|"
                r"студент\w*|мальчик\w*|девочк\w*|подросток\w*|пенсионер\w*|бала\w*|адам\w*|ағайын\w*)"
                r"(?!\s*(?:часов|часах|часы|телефон\w*|денег|вещ\w*|докумен\w*|сумк\w*|ключ\w*))")
_MISSING_PERSON_RX = _refix.compile(
    rf"{_MISSING_ROOT}(?:\s+\S+){{0,4}}\s+{_PERSON_WORD}|{_PERSON_WORD}(?:\s+\S+){{0,4}}\s+{_MISSING_ROOT}"
    rf"|помогите\s+найти|нужна\s+помощь\s+в\s+поиск\w*",
    _refix.I)

MISSING_PERSON_MAX_AGE_DAYS = 30  # старые архивные "пропал"-упоминания уже неактуальны для "поделитесь"
...
def build(posts, all_feed, stats):
    ...
    # Старые архивные упоминания (месяцы давности) не актуальны для "поделитесь, если можете".
    missing_sorted = [
        p for p in feed_sorted
        if is_missing_person_post(p.get("text", ""))
        and (d := parse_dt(p.get("created_at", ""))) and (now - d).days <= MISSING_PERSON_MAX_AGE_DAYS
    ][:40]
